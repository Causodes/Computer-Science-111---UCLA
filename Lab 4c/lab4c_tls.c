...
int main(int argc, char **argv) {
	...
	if(context == NULL) {
		fprintf(stderr, "Error, could not load SSL context.\n");
		exit(1);
	}
	...
	if(SSL_connect(ssl) != 1) {
		fprintf(stderr, "Error, cannot start TLS session.\n");
		exit(1);
	}
	...
	while(1) {
        ...
        if (pollfd[0].revents & POLLIN)
        {
            ...
            int num_bytes = SSL_read(ssl, buffer, 512);
		...
		if (clock.tv_sec >= next && start_stop) {
			...
			SSL_write(ssl, output_buffer, strlen(output_buffer));
			if(log_flag){
